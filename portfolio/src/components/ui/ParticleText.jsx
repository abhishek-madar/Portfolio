...
const ParticleText = ({
  ...
  useEffect(() => {
    ...
    const sampleText = async () => {
      ...
      const resolvedColor = color === 'inherit' ? computed.color : color;
      const resolvedHighlight = highlightColor === 'inherit' ? computed.color : highlightColor;
      
      const maxParticles = Math.max(1200, Math.min(15000, Math.floor((width * height) / 40)));
      const stride = Math.max(1, Math.ceil(targets.length / maxParticles));
      const baseRgb = hexToRgb(resolvedColor);
      const highlightRgb = hexToRgb(resolvedHighlight);
